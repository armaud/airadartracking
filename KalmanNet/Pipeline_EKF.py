...
class Pipeline_EKF:
    ...
    def NNTrain(self, n_Examples, train_input, train_target, n_CV, cv_input, cv_target, randomInit = False, cv_init=None,train_init=None, allStates=False):
        ...
        for ti in range(0, self.N_steps):
            ...
            for j in range(0, self.N_CV):
                ...
                for t in range(0, self.ssModel.T_test):
                    if torch.isnan(y_cv[:, t]).any():
                        print(f"NaN detected in Input CV sequence {j} at time step {t}")
                    x_out_cv[:, t] = self.model(y_cv[:, t])
                    if torch.isnan(x_out_cv[:, t]).any():
                        print(f"NaN detected in Output CV sequence {j} at time step {t}")
                        # print(f"Input: {y_cv[:, t]}")
                        # break
                ...
                if(allStates):
                    ...
                    loc = torch.tensor([True,False,True,False,True,False]) # for position only
                    MSE_cv_linear_batch[j] = self.loss_fn(x_out_cv[loc,:], cv_target[j, loc, :]).item()
            ...
            for j in range(0, self.N_B):
                ...
                for t in range(0, self.ssModel.T):
                    ...
                    if torch.isnan(x_out_training[:, t]).any():
                         print(f"NaN detected in training sequence {n_e} at time step {t}")
    ...
    def NNTest(self, n_Test, test_input, test_target, randomInit=False, test_init=None):
        ...
        for j in range(0, self.N_T):
            ...
            if (randomInit):
                if(test_init==None):
                    ...
                    self.model.InitSequence(test_init[j], self.ssModel.T_test)
            else:
                self.model.InitSequence(self.ssModel.m1x_0, self.ssModel.T_test)
