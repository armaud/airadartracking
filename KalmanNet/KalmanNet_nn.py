...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitSequence(self, M1_0, T):
        if torch.isnan(M1_0).any():
            print(f"NaN detected in Initial State")
        self.T = T
    ...
    def step_prior(self):
        ...
        if torch.isnan(self.m1x_prior).any():
            print(f"NaN detected in Prior CV sequence")
                    

        # Predict the 1-st moment of y
        self.m1y = torch.squeeze(self.h(self.m1x_prior))
        if torch.isnan(self.m1y).any():
            print(f"NaN detected in Predicted Measurement")
    ...
    def step_KGain_est(self, y):
        ...
        if torch.isnan(self.KGain).any():
            print(f"NaN detected in Kalman Gain")
    ...
    def KNet_step(self, y):
        ...
        if torch.isnan(INOV).any():
            print(f"NaN detected in Innovation")
        self.m1x_posterior_previous = self.m1x_posterior
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        if torch.isnan(out_FC2).any():
            print(f"NaN detected in FC2")
    ...
    #########################
    ### Checks for nan during back prop ###
    #########################
    def check_weights(self):
        if torch.isnan(self.FC1[0].weight).any():
            print("NaN in FC1 weight")
        if torch.isnan(self.FC1[0].bias).any():
            print("NaN in FC1 bias")
        if torch.isnan(self.FC2[0].weight).any():
            print("NaN in FC2 weight")
        if torch.isnan(self.FC2[0].bias).any():
            print("NaN in FC2 bias")
        if torch.isnan(self.FC3[0].weight).any():
            print("NaN in FC3 weight")
        if torch.isnan(self.FC3[0].bias).any():
            print("NaN in FC3 bias")
        if torch.isnan(self.FC4[0].weight).any():
            print("NaN in FC4 weight")
        if torch.isnan(self.FC4[0].bias).any():
            print("NaN in FC4 bias")
        if torch.isnan(self.FC5[0].weight).any():
            print("NaN in FC5 weight")
        if torch.isnan(self.FC5[0].bias).any():
            print("NaN in FC5 bias")
        if torch.isnan(self.FC6[0].weight).any():
            print("NaN in FC6 weight")
        if torch.isnan(self.FC6[0].bias).any():
            print("NaN in FC6 bias")
        if torch.isnan(self.FC7[0].weight).any():
            print("NaN in FC7 weight")
        if torch.isnan(self.FC7[0].bias).any():
            print("NaN in FC7 bias")
        return