...
def EKFTest(SysModel, test_input, test_target, allStates=True, randomInit = False,test_init=None):
    ...
    # MSE per time step
    MSE_EKF_linear_time_avg = torch.zeros(T)
    ...
    for sequence_target,sequence_input in zip(test_target,test_input):
        ...
        if(allStates):
            ...
            # Calculate per time step MSE (mean over states)
            MSE_EKF_linear_time_avg = torch.mean((EKF.x - sequence_target)**2, dim=0)
        else:
            ...
            # Calculate per time step MSE for selected states
            MSE_EKF_linear_time_avg = torch.mean((EKF.x[loc,:] - sequence_target[loc,:])**2, dim=0)
